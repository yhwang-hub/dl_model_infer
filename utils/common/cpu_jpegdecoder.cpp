...
#include "../preprocess/pre_process.h"
...
int decode_cpu(const std::vector<std::vector<char>> &files_data, uchar* out_imgs, size_t width, size_t height)
{
    ...
    CHECK(cudaMalloc(&temp_gpu, width * height * 3));
    for (size_t i = 0; i < files_data.size(); i++)
    {
        ...
        CHECK(cudaMemcpy(temp_gpu, temp, width * height * 3, cudaMemcpyHostToDevice));
        ai::preprocess::convert_RGBHWC_to_BGRCHW(temp_gpu, out_imgs + i * width * height * 3, 3, height, width);
