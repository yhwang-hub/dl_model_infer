    ...
    for (size_t i = 0; i < file_data.size(); i++) {
        ...
        if(img_width[i] != widths[0] || img_height[i] != heights[0]){
            ...
            for (int c = 0; c < channels; c++) {
                ...
                if (sz > isz[i].pitch[c]) {
                    if (ibuf[i].channel[c]) {
                        CHECK(cudaFree(ibuf[i].channel[c]));
                    }
                    CHECK(cudaMalloc((void **)&ibuf[i].channel[c], sz));
                    isz[i].pitch[c] = sz;
                }
            }
        }
...
int get_img(const uchar *d_chanR, int pitchR, 
    ...
    CHECK(cudaMemcpy2D(img + 0 * step, (size_t)width, d_chanR, (size_t)pitchR,
                               width, height, cudaMemcpyDeviceToDevice));
    CHECK(cudaMemcpy2D(img + 1 * step, (size_t)width, d_chanG, (size_t)pitchG,
                               width, height, cudaMemcpyDeviceToDevice));
    CHECK(cudaMemcpy2D(img + 2 * step, (size_t)width, d_chanB, (size_t)pitchB,
                               width, height, cudaMemcpyDeviceToDevice));
...
void decode_single_image(const std::vector<char> &img_data, nvjpegImage_t &out, decode_params_t &params, 
    ...
    CHECK(cudaEventRecord(startEvent, params.stream));
    ...
    // CHECK(cudaStreamSynchronize(params.stream)); //TODO new add

    CHECK(cudaEventRecord(stopEvent, params.stream));

    CHECK(cudaEventSynchronize(stopEvent));
    CHECK(cudaEventElapsedTime(&loopTime, startEvent, stopEvent));
...
int nvjpegDecoder::decode(const std::vector<std::vector<char>> &files_data, uchar* out_imgs){    
    ...
    for(size_t i = 0; i < params.size(); i++){
        CHECK(cudaStreamCreate(&params[i].stream));
    }
    ...
    for(size_t i = 0; i < params.size(); i++){
        CHECK(cudaStreamDestroy(params[i].stream));
    }
    return EXIT_SUCCESS;
}
