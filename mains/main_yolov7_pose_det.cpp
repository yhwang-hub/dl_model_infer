#include "main_yolov7_pose_det.h"

void yolov7_pose_trt_inference(ai::arg_parsing::Settings *s)
{
    ai::utils::Timer timer;
    tensorrt_infer::yolov7_pose_infer::yolov7_pose_detector yolov7_pose_obj;
    yolov7_pose_obj.initParameters(s->model_path, s->score_thr);
    ...
    // 模型预热， 如果需要单张推理，请调用yolov7_pose_obj.forward
    for (int i = 0; i < s->number_of_warmup_runs; i++)
    {
        auto warmup_batched_result = yolov7_pose_obj.forwards(yoloimages);
    }
    ...
    {
        batched_result = yolov7_pose_obj.forwards(yoloimages);
    }
