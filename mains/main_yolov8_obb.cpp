#include "main_yolov8_obb.h"

void yolov8_obb_trt_inference(ai::arg_parsing::Settings *s)
{
    ai::utils::Timer timer; // 创建
    tensorrt_infer::yolov8_infer::yolov8_obb_detector yolov8_obb_obj;
    yolov8_obb_obj.initParameters(s->model_path, s->score_thr);
    ...
    {
        auto warmup_batched_result = yolov8_obb_obj.forwards(yoloimages);
    }

    ai::cvUtil::BatchRotateBoxArray batched_result;
    // 模型推理
    timer.start();
    for (int i = 0; i < s->loop_count; ++i)
    {
        batched_result = yolov8_obb_obj.forwards(yoloimages);
    }
    ...
    if (!s->output_dir.empty())
    {
        ...
        ai::cvUtil::draw_batch_rotaterectangle(images, batched_result, s->output_dir, s->dotalabels);
    }
}