...
namespace tensorrt_infer
{
    ...
    {
        bool yolov8_obb_detector::initParameters(const std::string& engine_file, float score_thr, float nms_thr)
        {
            ...
            {
                model_info->m_postProcCfg.num_classes_ = model_info->m_postProcCfg.bbox_head_dims_[2] - 5; // yolov8-obb
            }
            model_info->m_postProcCfg.IMAGE_MAX_BOXES_ADD_ELEMENT =\
                model_info->m_postProcCfg.MAX_IMAGE_BOXES * model_info->m_postProcCfg.NUM_ROTATEBOX_ELEMENT;
        ...
        yolov8_obb_detector::~yolov8_obb_detector()
        {
            CHECK(cudaStreamDestroy(cu_stream)); // 销毁cuda流
        }

        void yolov8_obb_detector::adjust_memory(int batch_size)
        {
            // 申请模型输入和模型输出所用到的内存
        ...
        void yolov8_obb_detector::preprocess_gpu(int ibatch, const Image& image,
        ...
        void yolov8_obb_detector::postprocess_gpu(int ibatch, cudaStream_t stream_)
        {
            ...
            decode_yolov8_obb_kernel_invoker(
                ...
                model_info->m_postProcCfg.NUM_ROTATEBOX_ELEMENT, stream_
            );

            // 对筛选后的框进行nms操作
            rotatebbox_nms_kernel_invoker(
                boxarray_device, model_info->m_postProcCfg.nms_threshold_, model_info->m_postProcCfg.MAX_IMAGE_BOXES,
                model_info->m_postProcCfg.NUM_ROTATEBOX_ELEMENT, stream_
            );
        }

        BatchRotateBoxArray yolov8_obb_detector::parser_box(int num_image)
        {
            BatchRotateBoxArray arrout(num_image);
            for (int ib = 0; ib < num_image; ib++)
            {
                ...
                RotateBoxArray& output = arrout[ib];
                output.reserve(count); // 增加vector的容量大于或等于count的值
                for (int i = 0; i < count; i++)
                {
                    float* pbox = parray + 1 + i * model_info->m_postProcCfg.NUM_ROTATEBOX_ELEMENT;
                    int label = pbox[6];
                    int keepflag = pbox[7];
                    if (keepflag == 1)
                    {
                        ...
                        output.emplace_back(
                            pbox[0], pbox[1], pbox[2], pbox[3], pbox[4], pbox[5], label
                        );
                    }   
                }               
            }
        ...
        RotateBoxArray yolov8_obb_detector::forward(const Image& image)
        {
        ...
        BatchRotateBoxArray yolov8_obb_detector::forwards(const std::vector<Image>& images)
        {
